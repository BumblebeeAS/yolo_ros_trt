...
def find_latest_models(model_dir: Path) -> dict[str, str]:
    ...
    for category, files in category_files.items():
        ...
        console.print(
            f"[blue]Latest[/blue] [bold cyan]{category}[/bold cyan] model: [green]{latest_file['filename']}[/green]"
        )
...
def compile_latest(
    model_dir: str = typer.Argument(
        ...
        help="Directory path containing the model files",
    ),
    categories: list[str] | None = typer.Option(
        None, "--category", "-c", help="Specific categories to compile (default: all)"
    ),
    force: bool = typer.Option(
        False,
        "--force",
        "-f",
        help="Force recompilation even if .onnx/.engine files exist",
    ),
    ...
    try:
        ...
        console.print(
            Panel(
                f"[bold]Searching for models in:[/bold] {model_path}",
                title="YOLOv11 Model Compiler",
                border_style="blue",
            )
        )
        ...
        if categories:
            filtered_models = {
                cat: path for cat, path in latest_models.items() if cat in categories
            }
            if not filtered_models:
                err_console.print(
                    f"[bold red]No models found for specified categories:[/bold red] {categories}"
                )
                raise typer.Exit(1)
        ...
        for category, model_path in models_to_compile.items():
            ...
            if dry_run:
                ...
                table.add_row(
                    category, model_filename, "[yellow]Would compile[/yellow]", reason
                )
            else:
                console.print(
                ...
                try:
                    ...
                    err_console.print(
                        f"[bold red]Failed to compile {category} model:[/bold red] {str(e)}"
                    )
        ...
        if dry_run:
            ...
            if total_models == 0:
                ...
                if skipped_models:
                    console.print(
                        f"[dim]{len(skipped_models)} models were already compiled and skipped[/dim]"
                    )
            else:
                console.print(
